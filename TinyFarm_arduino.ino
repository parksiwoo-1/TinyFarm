...
// 아두이노에서 반복되는 함수
// 코드를 새로 업로드하거나 전원을 끄지 않으면 이 loop함수가 반복됨
void loop() { 
  // put your main code here, to run repeatedly:
    timeout += 1; // timeout = 한 번 반복하는 주기
    if(timeout % 10 == 0) {

      char light_buffer[15] ={'\0'};
      char wat_buffer[15] ={'\0'};
      char soil_buffer[15] = {'\0'};
      char fan_buffer[15] ={'\0'};
      char door_buffer[15] ={'\0'};
   ...
   // put your main code here, to run repeatedly:
    ...
    //Serial.print(waterValue); Serial.print(",");
      ...
      post("/TinyIoT/TinyFarm/Sensors/Soil", "CIN", "", wat_buffer); // 토양수분을 TinyIoT에 post 요청

      humidity = dht.readhumidity();
      String str1 = String(humidity);

      temperature = dht.readTemperature();
      String str2 = String(temperature);

      post("/TinyIoT/TinyFarm/Sensors/humidity", "CIN", "", str1); // 습도를 TinyIoT에 post 요청

      post("/TinyIoT/TinyFarm/Sensors/Temperature", "CIN", "", str2); // 온도를 TinyIoT에 post 요청
      ...
      co2_ppm = RX9.cal_co2(EMF, THER) / 10;
      ...
      if(displayToggle == 1) {
        ...
        printLCD(0, 0, sData);
        memset(sData, 0x00, 64);
        sprintf(sData, "light%-04d soil%-04d", lightoutput, waterValue);
        printLCD(0, 1, sData);
      }
      else {
        memset(sData, 0x00, 64);
      ...
      sprintf(sData, "{ \"temp\":%02d,\"humidity\":%02d,\"light\":%-04d,\"water\":%-04d }", 
      (int)temperature, (int)humidity, lightoutput, waterValue);
    ...
    // 시리얼 포트로 제어를 테스트할때 사용하는 함수
  // 터미널에 입력하여 값을 제어할 수 있음
 ...
 // 자동급수 명령은 물높이와 물펌프 세팅을 정확히 하고난 다음에 활성화하기
  ...
  // 10000밀리세컨드에 한 번 실행한다. 이부분 변경 가능
  delay(10000);
